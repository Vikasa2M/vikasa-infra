...
**DOT:** `exdot`
**Deployment mode:** shared cluster (Kubernetes)
...
| `d7a` | kubernetes | `d7a` | `leaf-d7a.nats.vikasa.exdot:7422` |
| `d7b` | kubernetes | `d7b` | `leaf-d7b.nats.vikasa.exdot:7422` |

## Provisioning Order

1. Ensure the **central** cluster (`core`) is reachable and its operator/accounts exist (prerequisite).
2. Provision the **central** cluster's slice in `charts/core/` first (k8s: apply the Stream CRs, sync-wave 0; bare-metal: see install notes below).
3. Provision each **regional** cluster's slice in `charts/<id>/` (k8s: sync-wave 1; bare-metal slices stay under `clusters/<id>/`).
...
Each Kubernetes cluster's slice is a Helm chart under `charts/<id>/` (`Chart.yaml`, `values.yaml`, `templates/`); no `kustomization.yaml` is emitted in helm mode.

- **Kubernetes:** deploy with `helm install vikasa-<id> charts/<id>` (or render first: `helm template charts/<id> | kubectl apply -f -`; with Argo CD, point the Application at the chart path — the generated `argocd/<id>.yaml` already targets `charts/<id>` and is emitted only when `cmd/gen -argo-repo-url` is set). `values.yaml` defaults reproduce the generated knob values (`namespace`, `tlsIssuer`, `secretStore`, `prometheusRelease`); override per environment with `-f`.
...
- **Kubernetes:** create the `vikasa-ca` ClusterIssuer (your CA — self-signed, Vault, or an intermediate). cert-manager issues each cluster's server cert into secret `<cluster>-nats-server-tls` from the generated `charts/<id>/templates/certificate.yaml`; mount that secret into the NATS server pods and reference it in the server's TLS listeners.
...
- This deployment's clusters:
  ...
  - `d7a` (kubernetes) — 3-node R3 quorum
  - `d7b` (kubernetes) — 3-node R3 quorum
