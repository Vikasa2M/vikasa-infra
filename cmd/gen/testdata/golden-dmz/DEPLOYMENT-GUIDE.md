...
**DOT:** `exdot`
**Deployment mode:** per-cluster (Kubernetes)
...
| `d1a` | kubernetes | `d1a` | `leaf-d1a.nats.vikasa.exdot:7422` |
| `dmz` | kubernetes | `dmz` | `leaf-dmz.nats.vikasa.exdot:7422` |
...
| Name | Target |
|------|--------|
| `leaf-exdot-d1-0.nats.vikasa.exdot` | `leaf-d1a.nats.vikasa.exdot:7422` |
| `leaf-exdot-d1-8.nats.vikasa.exdot` | `leaf-d1a.nats.vikasa.exdot:7422` |
...
```
Expected streams:
- `VIKASA_EXDOT_CENTRAL_D1_D1_0`
- `VIKASA_EXDOT_CENTRAL_D1_D1_8`

**`d1a`** (`leaf-d1a.nats.vikasa.exdot:7422`):

```
nats --server leaf-d1a.nats.vikasa.exdot:7422 stream ls
```
Expected streams:
- `VIKASA_EXDOT_D1_D1_0`
- `VIKASA_EXDOT_D1_D1_8`

**`dmz`** (`leaf-dmz.nats.vikasa.exdot:7422`):

```
nats --server leaf-dmz.nats.vikasa.exdot:7422 stream ls
```
Expected streams:
- `VIKASA_EXDOT_DMZ`
...
Each regional partition stream sources its assigned cabinets' `VIKASA_BUFFER`:

- `VIKASA_EXDOT_D1_D1_0` — 5 cabinet(s)
- `VIKASA_EXDOT_D1_D1_8` — 5 cabinet(s)

Onboard incrementally: add cabinets to the inventory, rerun `cmd/gen`, and apply (NACK reconciles, or `nats stream update` performs the live source-add). Per-cabinet credentials/JWTs are provisioned by sub-project B (control plane / PKI), not generated here.

## External Sharing (DMZ)

The **DMZ** cluster (`dmz`) hosts the `VIKASA_EXDOT_DMZ` stream (wave 2 — provisioned last). It re-publishes a filtered subset of central traffic under public subjects for external consumers.

- Provision the DMZ cluster's slice after central and regional (wave 2); apply the Stream CRs in `clusters/dmz/`.
- The public subjects and per-consumer share names are listed in [`dmz-catalog.md`](dmz-catalog.md).
- Issue per-consumer subscribe-only JWTs scoped to each share's public subject (sub-project B / PKI). Consumers receive a credential that cannot publish or access internal subjects.
...
- This deployment's clusters:
  ...
  - `d1a` (kubernetes) — 3-node R3 quorum
  - `dmz` (kubernetes) — 3-node R3 quorum
