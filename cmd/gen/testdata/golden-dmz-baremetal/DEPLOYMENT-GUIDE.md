...
| `dmz` | bare-metal | `dmz` | `leaf-dmz.nats.vikasa.exdot:7422` |
...
- **`dmz`** — on hosts `exdot-dmz-1, exdot-dmz-2, exdot-dmz-3`: install `nats-server`, drop that host's
  `clusters/dmz/nats-<host>.conf` as `/etc/vikasa/nats.conf`, install
  `clusters/dmz/nats-server.service`, then `systemctl enable --now nats-server`.
  Create the streams with `nats stream add --config clusters/dmz/streams/<NAME>.json`.
...
- This deployment's clusters:
  ...
  - `dmz` (bare-metal) — 3-node R3 quorum
